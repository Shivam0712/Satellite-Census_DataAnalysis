...
df_comb['Rud']=df_comb['80']+df_comb['79']+df_comb['78']+df_comb['81']+df_comb['74']+df_comb['75']
df_comb['Adv']=df_comb['73']+df_comb['72']
...
for i in list_villCol:
    df_pca[i]=(df_pca[i]-df_pca[i].mean())/df_pca[i].std()
...
df_pca.loc[df_pca['labels']==0,'labels']=4
df_pca.loc[df_pca['labels']==2,'labels']=5
df_pca.loc[df_pca['labels']==3,'labels']=6
df_pca.loc[df_pca['labels']==1,'labels']=7


# In[ ]:


fp="/home/tempuser/Doc/Cluster/MainSourceWater/"
...
for i in list_distCol2:
    df_Dist[i]=(df_Dist[i]-df_Dist[i].mean())/df_Dist[i].std()
...
for i in list_distCol2:
    plot=ggplot(df_Dist,aes(x='labels', y=i))+geom_boxplot()
    plot.show()
...
for i in list_distCol2:
    plot=ggplot(df_Dist,aes(x='labels', y=i))+geom_boxplot()
    plot.save(fp+"Dist_"+str(j)+"_"+i+".png")
    j=j+1
...
factor= FactorAnalysis(n_components=12,random_state=101).fit(X)
print(pd.DataFrame(factor.components_,columns=var_list))#, '130', '131', '132', '133', '134', '135', '136','137','138','139']))
...
for i in range(len(a)):
    a[i]=round(a[i],3)
print(a)
print(pd.DataFrame(pca.components_,columns=var_list))


# In[ ]:


df_components=pd.DataFrame(pca.components_,columns=var_list)
...
for j in range(n_components):
    ...
    df_comb[new]=0    
    for i in var_list:
        df_comb[new]=df_comb[new]+df_components.loc[j,i]*df_comb[i]
